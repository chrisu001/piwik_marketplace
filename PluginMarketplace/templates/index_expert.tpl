		...
		<tr{if $plugin.isAppstoreAvail && (!$plugin.isInstalled || $plugin.info.version != $plugin.appstoreinfo.version) } class="highlighted"{/if}>
			...
			<td class="togl action-links">
			 ...
			 {if  $name != 'PluginMarketplace' 
			     ...
			     <a title="{$name}" href="?module=PluginMarketplace&action=remove&pluginNames[]={$name}&{$commonquery.query}" style="color:#FFF;text-decoration:none:font-size:80%" class="submit removeplug">{'APUA_Index_BTN_Remove'|translate}</a>
			 {/if}
			 </td>
		</tr>
