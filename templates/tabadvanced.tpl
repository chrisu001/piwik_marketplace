		...
		<table cellspacing="2">
			<tr>
				...
				<td><select name="release" id="release">
					...
					src="plugins/PluginMarketplace/images/ajax-loader.gif"
...
$(function() {
	...
	$('#release').change(function(){
         var release = $("#release").val();
          ...
          $('#loaderrelease').show();
          $.ajax({
              url: '?module=PluginMarketplace&action=config&release=' + release,
              async: true,
              success: function(data) {
            	  ...
            	  $('#loaderrelease').hide();       
            	  $('#release').show();
            	  preUIVersion() ? $('#pstabs').tabs({ selected: 0 }) : $('#pstabs').tabs( "option", "active" , 0 ) ; 
            	   // pm.unbind("intercomm");
                  }
          });
    ...
    $('#expert').click(function () {
        ...
        $('#expert').hide();
        $.ajax({
            url: '?module=PluginMarketplace&action=config&expert=' + expertmode,
            async: true,
            success: function(data) {
            	pm.unbind("intercomm");
            	var loc = window.location + '';
            	// reload page on tab 1 (manager)
            	window.location.href =  loc.replace(/#.+/, '')  + '#1';
